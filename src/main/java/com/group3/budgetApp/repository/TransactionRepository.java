...
public interface TransactionRepository extends JpaRepository<Transaction, Integer> {
    @Query(value = "SELECT * FROM Transaction t INNER JOIN Account a ON t.from_account_id = a.id OR t.to_account_id = a.id INNER JOIN PROFILE p ON p.user_id = a.user_id WHERE p.user_id = ?1 ORDER BY transaction_dt DESC", nativeQuery = true)
