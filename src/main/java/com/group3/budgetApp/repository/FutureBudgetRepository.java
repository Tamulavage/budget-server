...
public interface FutureBudgetRepository extends JpaRepository<FutureBudget, Integer> {
        @Query(value = "select org_id, org_name, freq_per_month , direction, max(jan) as january_amount ,max(feb) as february_amount, max(march) as march_amount, "
                        ...
                        + " CASE WHEN month=1 then amount end as jan, " + " CASE WHEN month=2 then amount end as feb, "
                        + " CASE WHEN month=3 then amount end as march, "
                        + " CASE WHEN month=4 then amount end as apr, " + " CASE WHEN month=5 then amount end as may, "
                        + " CASE WHEN month=6 then amount end as june, "
                        + " CASE WHEN month=7 then amount end as july, " + " CASE WHEN month=8 then amount end as aug, "
                        ...
                        + " CASE WHEN month=12 then amount end as december " + " from budget.future_accounting fa, "
